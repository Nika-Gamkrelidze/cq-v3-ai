...
export const en: Dict = {
  ...
  'bot.policy.general.hint': 'Questions within your field that the documents don’t cover get a short general answer that says it is general guidance — a hospital’s bot can say which kind of doctor treats a broken leg. Prices, deadlines and promises still come only from your documents. Questions unrelated to your business are not answered under this option: the bot steers the customer back on topic and counts the question (to answer those too, choose “Answer any question”). In an emergency the bot gives safety guidance and passes the conversation to a colleague.',
  ...
  'bot.policy.open': 'Answer any question — unrelated ones count toward the warning and the stop',
  'bot.policy.open.hint': 'Every question gets an answer: from your shared documents first, from general knowledge otherwise. Questions unrelated to your business get a short general answer and are counted; the warning and stop thresholds under “Off-topic questions” limit how many a customer can ask. Prices, deadlines and promises still come only from your documents. In an emergency the bot gives safety guidance and passes the conversation to a colleague. Each unrelated question costs one small AI call.',
  'bot.policy.open.confirm': 'Let the bot answer any question, including ones unrelated to your business? Those answers come from general knowledge, not from your documents, and nobody at your company approved them. Every unrelated question is answered and costs tokens until the conversation reaches the stop threshold under “Off-topic questions” — with the stop set to 0 there is no limit. Prices, deadlines and promises still come only from your documents.',
  'bot.policy.open.confirm.ok': 'Answer any question',
...
export const ka: Dict = {
  ...
  'bot.policy.open': 'ნებისმიერ კითხვაზე პასუხი — თემის გარეშე კითხვები ითვლება: ჯერ გაფრთხილება, მერე შეჩერება',
  'bot.policy.open.hint': 'ბოტი ყველა კითხვაზე პასუხობს: ჯერ დაშვებული დოკუმენტებიდან, თუ იქ პასუხი არ არის — ზოგადი ცოდნიდან. თქვენს ბიზნესთან დაუკავშირებელ კითხვებზე ის მოკლე ზოგად პასუხს იძლევა და ასეთ კითხვებს ითვლის; ასეთ კითხვების რაოდენობას ზღუდავს „თემის გარეშე კითხვების“ ბლოკში მითითებული გაფრთხილებისა და შეჩერების ზღვრები. ფასებს, ვადებსა და დაპირებებს ბოტი კვლავ მხოლოდ თქვენი დოკუმენტებიდან იღებს. გადაუდებელ შემთხვევაში ბოტი უსაფრთხოების რეკომენდაციას იძლევა და საუბარს კოლეგას გადასცემს. ყოველ ასეთ კითხვაზე ბოტი AI-ს ერთ მცირე მოთხოვნას უგზავნის.',
  'bot.policy.open.confirm': 'მივცე ბოტს უფლება, ნებისმიერ კითხვაზე უპასუხოს, მათ შორის ისეთებზე, რომლებიც თქვენს ბიზნესს არ ეხება? ეს პასუხები ზოგადი ცოდნას ეყრდნობა, არა თქვენს დოკუმენტებს, და თქვენს კომპანიაში არავის დაუმტკიცებია. ბოტი ბიზნესთან დაუკავშირებელ ყველა კითხვაზე პასუხობს და ტოკენებს ხარჯავს, სანამ საუბარი „თემის გარეშე კითხვების“ ბლოკში მითითებულ შეჩერების ზღვარს არ მიაღწევს — თუ ზღვარი 0-ია, შეზღუდვა არ არის. ფასებს, ვადებსა და დაპირებებს ბოტი კვლავ მხოლოდ დოკუმენტებიდან იღებს.',
  'bot.policy.open.confirm.ok': 'ნებისმიერ კითხვაზე პასუხის დაშვება',
  ...
  'bot.offtopic': 'თემის გარეშე კითხვები',
...
export const ru: Dict = {
  ...
  'bot.policy.open': 'Отвечать на любые вопросы — те, что не по теме, считаются: сначала предупреждение, затем остановка',
  'bot.policy.open.hint': 'Бот отвечает на каждый вопрос: сначала по открытым документам, а если ответа там нет — из общих знаний. На вопросы, не связанные с вашим бизнесом, бот даёт короткий общий ответ и считает их; сколько таких вопросов можно задать, ограничивают пороги предупреждения и остановки в блоке «Вопросы не по теме». Цены, сроки и обещания бот по-прежнему берёт только из ваших документов. В экстренной ситуации бот даёт рекомендации по безопасности и передаёт разговор коллеге. Каждый такой вопрос — один небольшой запрос к ИИ.',
  'bot.policy.open.confirm': 'Разрешить боту отвечать на любые вопросы, в том числе не связанные с вашим бизнесом? Такие ответы берутся из общих знаний, а не из ваших документов, и никто в вашей компании их не утверждал. Бот отвечает на каждый вопрос не по теме и тратит на него токены, пока разговор не дойдёт до порога остановки в блоке «Вопросы не по теме»; при пороге 0 ограничения нет. Цены, сроки и обещания бот по-прежнему берёт только из документов.',
  'bot.policy.open.confirm.ok': 'Разрешить ответы на любые вопросы',
